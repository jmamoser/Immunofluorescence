...
function[intintdapi_allwells,  avgnuccy5_allwells, wellName]= IFfunction(row,col, dir)
...
for site=1  %number of sites
   site
    
    wellName=[num2str(row),'_', num2str(col)];
    
    %load images
    dapi=single(imread([dir, num2str(row), '_', num2str(col), '_', num2str(site), '_DAPI_stain.tif']));  %the part in purple in quotes has to match file name
...
%     cy3=single(imread([dir, num2str(row), '_', num2str(col), '_', num2str(site), '_Cy3_1.tif']));
    cy5=single(imread([dir, num2str(row), '_', num2str(col), '_', num2str(site), '_Cy5_stain.tif']));

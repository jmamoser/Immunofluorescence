%% The function detects blobs in the image.
function nuc_mask = blobdetector_4(nuc_raw,nucr,threshold,debrisarea)
imBlurred=imfilter(nuc_raw,fspecial('gaussian',5,5),'symmetric');% applying guassian filter
sharpMask=nuc_raw-imBlurred; %imagesc(sharpMask); colorbar;making a mask by subtracting the filtered image from the raw image
